...
* **General purpose:** Must handle any allocation size (from 1 byte to gigabytes), leading to internal fragmentation and bookkeeping overhead.
* **Slow syscalls:** When heap expansion is required, `malloc` transitions between user space and kernel space (`brk`/`mmap`), which is slow and non-deterministic.
...
### Allocate
Advances the offset pointer forward by the requested allocation size.
...
### Allocate
Advances the offset pointer and writes an allocation header immediately before the returned memory block.
...
## Pool allocator
Splits a memory arena into equal, fixed-size chunks. It provides fast allocations and deallocations with zero internal fragmentation for uniform objects.
